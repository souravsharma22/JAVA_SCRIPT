...
app.get("/filter", (req, res)=>{
  ...
  const typeJokes = jokes.filter((j)=> j.jokeType === query)
  res.json(typeJokes);
})
...
app.put("/jokes/:id", (req, res)=>{
  ...
  const targetIndex = jokes.findIndex((jokes)=> jokes.id === id)
  jokes[targetIndex] = replaceJoke;
...
app.delete("/jokes/:id",(req,res)=>{
  ...
  if(delIndex>-1){
    ...
    res.status(404);
    res.json({error: `Joke with the id ${id} not found, no joke is deleted`});
  }
